    ...
    <Fragment>
      ...
      <div className='flex w-full flex-col gap-12 px-28 pb-20 pt-14 max-2xl:px-14 max-lg:gap-8 max-lg:px-8 max-sm:px-6 max-sm:pt-6'>
        ...
        <AppointmentsTable session={session} tableData={{ data: filteredData, columns: columns }}>
          <div
            className={`grid w-full min-w-[1000px] content-end max-2xl:min-w-full max-2xl:max-w-full max-2xl:grid-cols-1 max-md:grid-cols-1 ${isUserAuthorized ? 'max-w-[50%]' : 'max-w-[600px]'} gap-2 ${isUserAuthorized ? 'grid-cols-4' : 'grid-cols-2'}`}
          >
